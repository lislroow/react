...
const Page = () => {
  ...
  const searchSatelliteReqDef: SatelliteSearchReq = {
    name: '',
    page: 0,
    size: PageSizeOptions[0],
  };
  const [ searchParams, setSearchParams ] = useState<SatelliteSearchReq>(searchSatelliteReqDef);
  const [ pageInfoRes, setPageInfoRes ] = useState<PageInfoRes>();
  const [ starSearchResList, setSatelliteSearchResList ] = useState<SatelliteSearchRes[]>([]);
  ...
  useEffect(() => {
    ...
    const parsedParams = Object.keys(searchParams).reduce((acc, key) => {
      ...
      return acc;
    }, {} as SatelliteSearchReq);

    let params = null;
    if (Object.keys(parsedParams).length > 0) {
      params = {...searchParams, ...parsedParams};
    } else {
      params = searchSatelliteReqDef;
    }
    ...
    JpaSampleService.getSatellitesSearch(params)
      .then((response) => {
        ...
        setSatelliteSearchResList(response.data.content);
      });
  }, [query]);
